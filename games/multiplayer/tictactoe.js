...
function updateGrid(newGrid) {
    ...
    for (let row = 0; row < diff; row++) {
        for (let col = 0; col < diff; col++) {
            ...
            if (button) {
                button.innerText = ((_a = newGrid[row][col]) === null || _a === void 0 ? void 0 : _a.toString()) || '-';
                if (button.innerText !== "-")
...
function winner() {
    if (grid.every(val => val.every(n => !!n))) {
        ...
        if (isMatch) {
            combo.forEach(n => { var _a; return (_a = document.getElementById(`sq${n[0] * diff + n[1]}`)) === null || _a === void 0 ? void 0 : _a.setAttribute('style', 'color: blue;'); });
            const currentElem = document.getElementById('current');
            if (currentElem)
                currentElem.innerText = `The winner is: ${current}`;
            return true;
        }
        else
            return false;
